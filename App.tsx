...
const shortLink = "https://www.paypal.com/ncp/payment/Z937SK69DRWS8";
const longLink = "https://www.paypal.com/ncp/payment/XRWM23U4Y5QPU";
const upiLink = "upi://pay?pa=jpofficial12345-2@oksbi&pn=Jiten%20patel&aid=uGICAgIDj3rSgHA";
...
const BubuDuduOrderApp = () => {
  ...
  const canProceed = () => {
    if (step === 1) {
      ...
      if (formData.deliverySpeed === "Custom Date" && !formData.customDate)
        return false;
      return true;
    }
    if (step === 2) {
      if (
        ...
        formData.customScript.length < 10
      )
        return false;
  ...
  const handlePayment = (type) => {
    ...
    const selectedLink = formData.length.includes("Short")
      ? shortLink
      : longLink;
    ...
    emailjs
      .send(
        ...
        "template_ztr6rdq",
        {
          ...
          scriptChoice: formData.scriptChoice,
          customScript: formData.customScript,
          shortIdea: formData.shortIdea,
      ...
      .then(() => {
        ...
        window.location.href = type === "upi" ? upiLink : selectedLink;
      })
      .catch((error) => {
        setIsSubmitting(false);
        // This will now show the exact error on your tablet!
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="bg-rose-50 border border-rose-100 rounded-xl p-4 flex items-start gap-3 text-rose-800">
        ...
        <div>
          ...
          <p className="text-sm mt-1">
            Please note for your story ideas that the{" "}
            <strong className="text-white bg-rose-300 px-1 rounded">
              White
            </strong>{" "}
            character is <strong>Bubu</strong> and the{" "}
            <strong className="text-amber-100 bg-amber-700 px-1 rounded">
              Brown
            </strong>{" "}
            character is <strong>Dudu</strong>.
          </p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              "Short Video (approx. 40 seconds)",
              "Long Video (approx. 2 minutes)",
            ].map((opt) => (
              <button
                key={opt}
  ...
  const renderStep2 = () => (
    <div className="space-y-6 animate-fade-in">
      <div className="flex bg-gray-100 p-1 rounded-xl">
        {["I will write the script", "I can’t write the script myself"].map(
          (choice) => (
            <button
              key={choice}
        ...
        <div className="animate-fade-in space-y-4">
          <div className="bg-rose-50 border border-rose-100 rounded-xl p-4 text-rose-800 text-sm">
            <p className="font-medium">
              You can just share a short idea or context, and we’ll write it for
              you 👍
            </p>
            <p className="opacity-80 mt-1">
              Once you approve the script, we’ll start working ✨
            </p>
          </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <textarea
                ...
                onChange={(e) => updateForm("shortIdea", e.target.value)}
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="bg-amber-50 border border-amber-200 rounded-xl p-5 text-amber-900">
        ...
        <p className="text-sm leading-relaxed">
          ...
          <br />
          We may not be available after 1–2 weeks due to ongoing projects.
          Secure your slot now to avoid delays! 💖
        </p>
      </div>
    ...
    <div className="space-y-6 animate-fade-in">
        ...
        <div className="p-5 space-y-4 border-b border-gray-100">
          <div className="flex justify-between items-center text-sm">
            ...
            <span className="font-medium text-gray-900">
              {formData.length.split(" (")[0]}
            </span>
          </div>
          <div className="flex justify-between items-center text-sm">
            <span className="text-gray-500">Format</span>
            <span className="font-medium text-gray-900">
              {formData.orientation.split(" - ")[0]}
            </span>
          </div>
          <div className="flex justify-between items-center text-sm">
      ...
      <div className="bg-rose-50 rounded-xl p-5 border border-rose-100 text-center">
        <p className="text-sm text-rose-800 mb-2">
          Advance Payment Required to Begin
        </p>
        <p className="text-3xl font-black text-rose-600">
          {pricing.currency}
          {pricing.advance}
        </p>
        <p className="text-xs text-rose-600/70 mt-2 uppercase tracking-wide font-semibold">
          50% of Estimated Total
        </p>
      </div>

      <p className="text-xs text-center text-gray-400 px-4">
        Disclaimer: Final price may vary depending on script complexity. The
        remaining 50% will be collected via a manual invoice after the
        watermarked preview is sent.
      </p>
    </div>
  );
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="space-y-3">
        <button
          ...
          className="w-full flex items-center justify-between p-4 bg-[#003087] hover:bg-[#002060] text-white rounded-xl transition-colors"
        >
        ...
        <button
          ...
          className="w-full flex items-center justify-between p-4 bg-[#003087] hover:bg-[#002060] text-white rounded-xl transition-colors"
        >
          <span className="font-medium">Pay with PayPal / Card</span>
          <ChevronRight className="w-5 h-5 opacity-70" />
        </button>
        ...
        {formData.country === "India" && (
          <button
            ...
            className="w-full flex items-center justify-between p-4 bg-green-600 hover:bg-green-700 text-white rounded-xl transition-colors"
          >
        ...
        <button
          ...
          className="w-full flex items-center justify-between p-4 border border-gray-200 hover:bg-gray-50 text-gray-700 rounded-xl transition-colors"
        >
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl overflow-hidden flex flex-col min-h-[600px]">
        <div className="bg-gray-50 px-6 py-4 border-b border-gray-100 flex justify-between relative">
          ...
          {steps.map((s, i) => {
            ...
            return (
              <div
                key={i}
                className="flex flex-col items-center gap-1 z-10 relative bg-gray-50 px-1"
              >
                <div
        ...
        {step < 5 && (
          <div className="p-4 border-t border-gray-100 flex justify-between bg-white">
            <button
              onClick={handleBack}
              className={`px-4 py-2 font-medium text-gray-500 hover:text-gray-800 transition-opacity ${
